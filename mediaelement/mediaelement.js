...
mejs.HtmlMediaElementShim = {
	...
	create: function(el, o) {
		...
		if (playback.method == 'native') {
			...
			if (htmlMediaElement._updated) {
				return htmlMediaElement
			} else {
				htmlMediaElement._updated = true
				return this.updateNative( htmlMediaElement, options, autoplay, preload, playback);
			}
		} else if (playback.method !== '') {
			// create plugin to mimic HTMLMediaElement
			if (htmlMediaElement._pluginId) {
				var plugin = mejs.MediaPluginBridge.pluginMediaElements[htmlMediaElement._pluginId]
				console.log(htmlMediaElement._pluginId, plugin)
				return plugin
			} else {
				var plugin = this.createPlugin( htmlMediaElement, options, isVideo, playback.method, (playback.url !== null) ? mejs.Utility.absolutizeUrl(playback.url) : '', poster, autoplay, preload, controls);
				htmlMediaElement._pluginId = plugin.id
				console.log(plugin, htmlMediaElement._pluginId)
				return plugin
			}
		} else {
